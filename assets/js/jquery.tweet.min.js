  ...
  e.fn.tweet = function(t) {
    ...
    function u(e, t) {
      return e.replace(r, function(e) {
        var n = /^[a-z]+:/i.test(e) ? e : "http://" + e, r = e;
    ...
    function f(e) {
      ...
      }, n = new Date, r = t((n.getTime() - e) / 1e3);
      r < 1 && (r = 0);
      return {
        days: t(r / 86400),
        hours: t(r / 3600),
    ...
    function h() {
      var e = "https:" === document.location.protocol ? "https:" : "http:", t = n.fetch === null ? n.count : n.fetch, r = "&include_entities=1&callback=?";
    ...
    function p(e, t) {
      return t ? "user" in e ? e.user.profile_image_url_https : p(e, !1).replace(/^http:\/\/[a-z0-9]{1,3}\.twimg\.com\//, "http://api.twitter.com/1/statuses/user_timeline/envato.json") : e.profile_image_url || e.user.profile_image_url;
    }
    function d(t) {
      var r = {};
      r.item = t;
      ...
      r.join_text = n.join_text === "auto" ? c(t.text) : n.join_text;
      ...
      r.tweet_relative_time = l(f(r.tweet_time));
      ...
      r.retweeted_tweet_text = e([ u(t.text, r.entities) ]).linkUser().linkHash()[0];
      r.tweet_text_fancy = e([ r.tweet_text ]).makeHeart()[0];
      r.avatar_size = n.avatar_size;
      r.avatar_url = p(r.retweet ? t.retweeted_status : t, document.location.protocol === "https:");
      r.avatar_screen_name = r.retweet ? r.retweeted_screen_name : r.screen_name;
      r.avatar_profile_url = r.twitter_base + r.avatar_screen_name;
      r.user = i('<a class="tweet_user" href="{user_url}">{screen_name}</a>', r);
      r.join = n.join_text ? i('<span class="tweet_join">{join_text}</span>', r) : "";
      r.avatar = r.avatar_size ? i('<a class="tweet_avatar" href="{avatar_profile_url}"><img src="{avatar_url}" height="{avatar_size}" width="{avatar_size}" alt="{avatar_screen_name}\'s avatar" title="{avatar_screen_name}\'s avatar" border="0"/></a>', r) : "";
      ...
      r.retweeted_text = i('<span class="tweet_text">{retweeted_tweet_text}</span>', r);
    ...
    function v(t, r) {
      ...
      e(t).empty().append(s);
      n.intro_text && s.before('<p class="tweet_intro">' + n.intro_text + "</p>");
      n.outro_text && s.after('<p class="tweet_outro">' + n.outro_text + "</p>");
      e(t).trigger("loaded").trigger(r.length === 0 ? "empty" : "full");
      n.refresh_interval && window.setTimeout(function() {
        e(t).trigger("tweet:load");
      }, 1e3 * n.refresh_interval);
    }
    function m(t) {
      var r = e('<p class="loading">' + n.loading_text + "</p>");
      n.loading_text && e(t).not(":has(.tweet_list)").empty().append(r);
      e.getJSON(h(), function(r) {
        var i = e.map(r.results || r, d);
        i = e.grep(i, n.filter).sort(n.comparator).slice(0, n.count);
        e(t).trigger("tweet:retrieved", [ i ]);
      });
    }
    var n = e.extend({
      username: null,
    ...
    }, t), r = /\b((?:https?:\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?«»“”‘’]))/gi;
    e.extend({
      tweet: {
        t: i
      }
    ...
    return this.each(function(t, r) {
      n.username && typeof n.username == "string" && (n.username = [ n.username ]);
      e(r).unbind("tweet:render").unbind("tweet:retrieved").unbind("tweet:load").bind({
        "tweet:load": function() {
          m(r);
        },
